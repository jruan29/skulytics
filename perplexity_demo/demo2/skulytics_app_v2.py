...
<style>
    ...
    * {
    ...
    /* Professional KPI Tile - Updated to remove border unless matches indicator */
    .kpi-tile {
        background: white;
        border: 1px solid #E2E8F0;
        border-radius: 6px;
...
def show_chat_panel(context_text):
    """Display AI chat panel with conversation flow"""
...
def render_main_dashboard():
    ...
    # TOP 5-6 MOST IMPORTANT KPIs IN ONE ROW
    ...
    with kpi_row[5]:
        # Total Media Spend - Fixed to be neutral/orange color since it's just spending
        st.markdown("""
    ...
    with chart_cols[1]:
        ...
        # UPDATED: Aggregate by Category with drill-down capability
        ...
        # FIX: Changed update_xaxis/update_yaxis to update_xaxes/update_yaxes
    ...
    # Commerce Journey - UPDATED to Bar Trends + Top 10 Heatmap
    ...
    with journey_cols[0]:
        ...
        # Create mini trend bars for each stage
    ...
    with journey_cols[1]:
        ...
        # Create heatmap with top 10 products
        top10 = top_products.head(10).copy()
        top10 = top10.merge(sku_data[['sku_id', 'sku_name']], on='sku_id')
        
        # Get performance metrics for heatmap (normalized 0-100 for visualization)
        heatmap_data = []
        for sku_id in top10['sku_id']:
            ...
            if perf is not None and content is not None:
                heatmap_data.append({
                    'SKU': sku_id,
                    'CVR': perf['cvr'],
                    'Rank': 11 - perf['organic_rank'],  # Invert so #1 = highest
                    'Content': content['overall_health'],
                    'OOS': 100 - perf['oos_pct']  # Invert so 0% OOS = 100
                })
    ...
    # AI Chat Panel (if activated)
    if st.session_state.show_chat:
        st.markdown("---")
        render_chat_panel()

def render_chat_panel():
    """Render AI chat conversation panel"""
    st.markdown("""
    <div class="chat-container">
        <div class="chat-header">
            <div><i class="fas fa-comments"></i> AI Assistant</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if "SKU003" in context:
        ...
        <div class="message-ai">
            ...
            • <strong>Strategic:</strong> Improve content (add lifestyle images, expand description)
        </div>
        """, unsafe_allow_html=True)
        
        btn_cols = st.columns(3)
        with btn_cols[0]:
            if st.button("Simulate recovery scenarios", key="chat_btn1"):
                st.session_state.selected_module = "Product Performance"
                st.session_state.show_chat = False
                st.rerun()
        with btn_cols[1]:
            if st.button("Show content improvements", key="chat_btn2"):
                st.session_state.selected_module = "Content Crafter"
                st.session_state.show_chat = False
                st.rerun()
        with btn_cols[2]:
...
def render_product_performance():
    ...
    # KPI Metadata Tiles - 6 tiles in 2 rows
...
# =============================================================================
# MODULE 3: CONTENT CRAFTER (FIXED - No raw HTML display)
# =============================================================================
def render_content_crafter():
    ...
    # Content Health Scores - FIXED: Proper formatting
    ...
    for i, (label, score, color) in enumerate(scores):
        with score_cols[i]:
            ...
            # FIXED: Proper HTML without nested divs
            st.markdown(f"""
    ...
    # 3-Column Layout - FIXED: Display actual content, not HTML code
